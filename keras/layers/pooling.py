...
class _PoolingIndex2D(Layer):
    """Abstract class for different pooling 2D layers with two outputs, 
       i.e. the pooling maps and the pooling indices's.
    """

    def __init__(self, pool_size=(2, 2), strides=None, padding='valid',
                 data_format=None, **kwargs):
        super(_PoolingIndex2D, self).__init__(**kwargs)
        data_format = conv_utils.normalize_data_format(data_format)
        if strides is None:
            strides = pool_size
        self.pool_size = conv_utils.normalize_tuple(pool_size, 2, 'pool_size')
    ...
    def call(self, inputs):
        output, idx = self._pooling_function(inputs=inputs,
                                        pool_size=self.pool_size,
                                        strides=self.strides,
                                        padding=self.padding,
                                        data_format=self.data_format)
...
class MaxPoolingWithIndex2D(_Pooling2D):
    """Max pooling operation for spatial data with pooling indices's
    ...
    # Output shape
        - If `data_format='channels_last'`:
            List with two 4D tensor with shape:
            `(batch_size, pooled_rows, pooled_cols, channels)`
        - If `data_format='channels_first'`:
            List with two 4D tensor with shape:
             `(batch_size, channels, pooled_rows, pooled_cols)`
    ...
    ''' todo: do we need the interface line
    '''
    #@interfaces.legacy_pooling2d_support
    def __init__(self, pool_size=(2, 2), strides=None, padding='valid',
                 data_format=None, **kwargs):
    ...
    def _pooling_function(self, inputs, pool_size, strides,
                          padding, data_format):
        output, idx = K.pool2d_with_argmax(inputs, pool_size, strides,
                          padding, data_format)

        return K.concatenate([output, K.cast(idx, 'float32')], 1)


class MaxPoolingWithIndex2D_2out(_PoolingIndex2D):
    """Max pooling operation for spatial data with pooling indices's
    ...
    def __init__(self, pool_size=(2, 2), strides=None, padding='valid',
                 data_format=None, **kwargs):
        super(MaxPoolingWithIndex2D_2out, self).__init__(pool_size, strides, padding,
                                           data_format, **kwargs)
